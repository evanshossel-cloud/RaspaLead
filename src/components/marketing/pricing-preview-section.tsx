...
export function PricingPreviewSection() {
  return (
    <section id="planos" className="bg-[#f5f1e8] px-4 py-16 text-black md:px-6 lg:px-8">
      <div className="mx-auto max-w-7xl">
        <div className="flex flex-col justify-between gap-6 lg:flex-row lg:items-end">
          <div className="max-w-3xl">
            <p className="text-sm font-black uppercase tracking-[0.18em] text-[#6c35ff]">Planos</p>
        ...
        <div className="mt-10 grid gap-5 md:grid-cols-2 xl:grid-cols-4">
          {plans.map((plan) => (
            <article key={plan.name} className="rounded-[20px] border-4 border-black bg-white p-5 shadow-[8px_8px_0_#000]">
              ...
              <div className="mt-6 space-y-3">
                {plan.features.map((feature) => (
                  <div key={feature} className="flex items-center gap-2 text-sm font-bold">
                    <CheckCircle2 className="h-5 w-5 text-[#119b52]" />
                    {feature}
                  </div>
                ))}
              </div>
              <Link
                href="/register"
                className="mt-7 inline-flex w-full items-center justify-center gap-2 rounded-[12px] border-[3px] border-black bg-[#111] px-4 py-3 text-sm font-black uppercase text-white shadow-[4px_4px_0_#000]"
              >
                Entrar na beta
